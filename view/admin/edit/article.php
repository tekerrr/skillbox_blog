...
<div class="container">
    ...
    <form class="form pt-4" enctype="multipart/form-data" action="" method="post">
        ...
        <input type="hidden" name="id" value="<?=$_POST['id'] ?? $article['id'] ?? ''?>">
        ...
        <div class="form-group">
            ...
            <input type="text" id="<?=$inputName?>" name="<?=$inputName?>"
                   ...
                   value="<?=$_POST[$inputName] ?? $article['title'] ?? ''?>">
        ...
        <div class="form-group">
            ...
            <textarea class="form-control rounded-0 col <?=$validStatus = $status[$inputName] ?? ''?>"
                      name="<?=$inputName?>" id="<?=$inputName?>" rows="3"><?=$_POST[$inputName] ?? $article['abstract'] ?? ''?></textarea>
            <?php include VIEW_TEMPLATE . 'form_invalid_message.php' ?>
        </div>
        ...
        <div class="form-group">
            ...
            <textarea class="form-control rounded-0 col <?=$validStatus = $status[$inputName] ?? ''?>"
                      name="<?=$inputName?>" id="<?=$inputName?>" rows="10"><?=$_POST[$inputName] ?? $article['text'] ?? ''?></textarea>
            <?php include VIEW_TEMPLATE . 'form_invalid_message.php' ?>
        </div>
        ...
        <div class="row form-group mx-0">
            <input type="submit" name="submit_edit_article" class="btn btn-primary btn-md rounded-0" value="Сохранить">
